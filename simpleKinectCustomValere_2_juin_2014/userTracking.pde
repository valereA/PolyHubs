...
void updateTracking() {
  ...
  userCount = kinect.getNumberOfUsers();   //our test for num# of users
  pushMatrix();
  textSize(60);
  text(userCount, 100, 100);
  popMatrix();
  if (userCount > 0) {  
    if (trackCoM) {
...
void trackAllUsersCenterOfMass() {
  ...
  if ( inventory.get(0)>0) {
    PVector position = new PVector();
    kinect.getCoM(inventory.get(0), position);
    userIds[0] = inventory.get(0); 
    userCoM.clear();
    userCoM.setAddrPattern("/CoM/1" );  // fff
    userCoM.add(new float[] {
      position.x, position.y, position.z
    }
    );
  ...
  }//fin  if(userList.size () == 1)
  if(userList.size () >= 2) {
  if ( inventory.get(1)>0) {
    PVector position = new PVector();
    kinect.getCoM(inventory.get(1), position);
    userIds[1] = inventory.get(1); 
    userCoM.clear();
    userCoM.setAddrPattern("/CoM/2" );  // fff
    userCoM.add(new float[] {
      position.x, position.y, position.z
    }
    );
