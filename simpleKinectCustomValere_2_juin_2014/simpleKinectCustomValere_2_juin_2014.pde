...
int[] trackJointArray = {
};// = new int[15]; //array of openni joint IDs.
int[] depthMap;
...
void draw() {
  ...
  //init visibleUsers at everyframe via void onVisibleUser
  visibleUsers = 0;

  if ( inventory.get(0)!=0) {
   // println(" inv 0 = " + inventory.get(0));
  }
  if ( inventory.get(1)!=0) {
   // println(" inv 1 = " + inventory.get(1));
  }
